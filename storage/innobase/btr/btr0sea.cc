...
static void btr_search_check_free_space_in_heap(const dict_index_t *index)
{
  ...
  mysql_rwlock_wrlock(&part->latch);
  ...
  mysql_rwlock_unlock(&part->latch);
}
	...
	mtr_t*		mtr)
{
	...
	if (!ahi_latch) {
		mysql_rwlock_rdlock(&part->latch);
...
{
	mysql_rwlock_t*	ahi_latch = &btr_search_sys.get_part(*cursor->index)
		->latch;
	...
	buf_block_t*	block)
{
	...
	mysql_rwlock_t* ahi_latch = index
